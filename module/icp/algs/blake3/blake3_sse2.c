/*
 * Version 1.3.3 from https://github.com/BLAKE3-team/BLAKE3
 *
 * These changes have been made to the original version:
 * - SIMDE headers used instead of native <immintrin.h>
 *    (https://github.com/simd-everywhere/simde)
 * - Some variables renamed to maintain a BLAKE3_* namespace for some more
 *    common names (MSG_SCHEDULE, IV)
 * - Exported functions prefixed with zfs_.
 * - Some macros undefined to avoid some collisions
 */
...
void zfs_blake3_compress_in_place_sse2(uint32_t cv[8],
                                   const uint8_t block[BLAKE3_BLOCK_LEN],
...
void zfs_blake3_compress_xof_sse2(const uint32_t cv[8],
                              const uint8_t block[BLAKE3_BLOCK_LEN],
                              uint8_t block_len, uint64_t counter,
...
INLINE void hash_one_sse2(const uint8_t *input, size_t blocks,
  ...
  while (blocks > 0) {
    ...
    zfs_blake3_compress_in_place_sse2(cv, input, BLAKE3_BLOCK_LEN, counter,
                                  block_flags);
...
void zfs_blake3_hash_many_sse2(const uint8_t *const *inputs, size_t num_inputs,
