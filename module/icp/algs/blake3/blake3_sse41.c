...
void zfs_blake3_compress_in_place_sse41(uint32_t cv[8],
                                    const uint8_t block[BLAKE3_BLOCK_LEN],
...
void zfs_blake3_compress_xof_sse41(const uint32_t cv[8],
                               const uint8_t block[BLAKE3_BLOCK_LEN],
                               uint8_t block_len, uint64_t counter,
...
static
void zfs_blake3_hash4_sse41(const uint8_t *const *inputs, size_t blocks,
...
INLINE void hash_one_sse41(const uint8_t *input, size_t blocks,
  ...
  while (blocks > 0) {
    ...
    zfs_blake3_compress_in_place_sse41(cv, input, BLAKE3_BLOCK_LEN, counter,
                                   block_flags);
...
void zfs_blake3_hash_many_sse41(const uint8_t *const *inputs, size_t num_inputs,
                            size_t blocks, const uint32_t key[8],
                            uint64_t counter, bool increment_counter,
                            uint8_t flags, uint8_t flags_start,
                            uint8_t flags_end, uint8_t *out) {
  while (num_inputs >= DEGREE) {
    zfs_blake3_hash4_sse41(inputs, blocks, key, counter, increment_counter, flags,
